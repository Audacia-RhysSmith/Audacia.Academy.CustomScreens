class GridfoxHttp {
    ...
    private gridfoxAppMessages(callback: (event: MessageEvent) => void, ignoreEventType: string = null) {
      window.addEventListener('message', (event) => {   
        const invalidOrigin = event.origin !== this.parentURL.origin;
        const invalidEventType = ignoreEventType ? event.data.type !== ignoreEventType : false;
    ...
    public async post(endpoint: string, payload: Record<string, any> | FormData): Promise<any> {
    ...
    public async put(endpoint: string, payload: Record<string, any> | FormData): Promise<any> {
      const isFormData = payload instanceof FormData;
      const headers = await this.getHeaders(isFormData);
      return new Promise((resolve, reject) => {
        fetch(this.url + endpoint, {
